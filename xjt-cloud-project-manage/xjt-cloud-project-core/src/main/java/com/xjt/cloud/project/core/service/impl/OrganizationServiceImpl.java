...
public class OrganizationServiceImpl extends AbstractService implements OrganizationService {
    ...
    public Data saveDepartment(String json) {
        ...
        String layerOrder = organizationDao.findOrgLayerOrderById(dep.getParentId());
        if(StringUtils.isNotEmpty(layerOrder)){
            layerOrder = layerOrder + "A" + dep.getId();
            dep.setLayerOrder(layerOrder);
            organizationDao.saveOrgLayerOrderById(dep);
        }
    ...
    public Data saveCompany(String json) {
        ...
        String layerOrder = "A" + company.getId();
        company.setLayerOrder(layerOrder);
        organizationDao.saveOrgLayerOrderById(company);
    ...
    @Override
    public Data findAllOrgLayerOrder() {
        Organization o = new Organization();
        o.setOrgType(2);
        List<Organization> oList = organizationDao.findAllOrgLayerOrder(o);
        if(oList != null && oList.size() > 0){
            String layerOrder = "";
            List<Organization> orgList;
            for(Organization co : oList){
                layerOrder = "A" + co.getId();
                co.setLayerOrder(layerOrder);
                organizationDao.saveOrgLayerOrderById(co);
                recilOrg(co);
            }
        }
        return null;
    }
    private void recilOrg(Organization co){
        co.setOrgType(3);
        List<Organization> orgList = organizationDao.findAllOrgLayerOrder(co);
        if(orgList != null && orgList.size() > 0){
            String layerOrder = "";
            for (Organization org : orgList){
                layerOrder = organizationDao.findOrgLayerOrderById(org.getParentId());
                layerOrder = layerOrder + "A" + org.getId();
                org.setLayerOrder(layerOrder);
                organizationDao.saveOrgLayerOrderById(org);
                recilOrg(org);
            }
        }
    }
