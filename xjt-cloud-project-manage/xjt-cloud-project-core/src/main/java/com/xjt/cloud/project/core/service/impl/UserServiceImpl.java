...
public class UserServiceImpl extends AbstractService implements UserService {
    ...
    public Data saveUser(String json) {
        ...
        user.setLoginName(StringUtils.isEmpty(user.getLoginName()) ? user.getPhone() : user.getLoginName());
        ...
        orgUser.initOrgUser(user.getOrgId(), user.getId(), projectId, user.getUserName(), user.getCertificateLevel(), user.getCertificateNumber(),
                user.getCertificate());
        oldOrgUser = orgUserService.findOrgUser(oldOrgUser);
        if(oldOrgUser!=null){
            return asseData(600,"用户已存在");
        }
            ...
            for (User phoneUser:phoneList){
                if (user.getPhone().equals(phoneUser.getPhone())){
                    if(null != phoneUser.getId()){
                        ...
                        password = StringUtils.isEmpty(ConstantsProjectMsg.DEFAULT_PASS) ? StringUtils.generateRanStr(6) : ConstantsProjectMsg.DEFAULT_PASS;//默认密码，如没有配置则随机生成密码
    ...
    private int registerUser(User user){
        String password = StringUtils.isEmpty(ConstantsProjectMsg.DEFAULT_PASS) ? StringUtils.generateRanStr(6) : ConstantsProjectMsg.DEFAULT_PASS;//默认密码，如没有配置则随机生成密码
        PasswordEncoder passwordEncoder = new StandardPasswordEncoder();//判断密码是否正确
        password = passwordEncoder.encode(password);
        user.setPassword(password);
        user.setLoginName(StringUtils.isEmpty(user.getLoginName()) ? user.getPhone() : user.getLoginName());
