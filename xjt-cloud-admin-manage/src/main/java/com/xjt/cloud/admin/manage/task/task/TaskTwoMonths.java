...
/**
 * @ClassName TaskTwoMonths
 * @Author dwt
 * @Date 2019-08-06 9:41
 * @Description 每两月周期任务处理
 * @Version 1.0
 */
@Configuration
@Component
public class TaskTwoMonths implements Job, Serializable {
    private static Logger logger = LoggerFactory.getLogger(TaskTwoMonths.class);
    @Override
    public void execute(JobExecutionContext jobExecutionContext){
        logger.info("ScheduleTask ====> 开启!" );
        String exceptionMsg = "任务执行失败";
        try {
            //类型  4:每两月 （每周日10点：0 0 10 ? * SUN）
            String res = HttpUtils.sendGet(ConstantsClient.PERIOD_TASK_HANDLE_URL,"json={\"type\":4}");
