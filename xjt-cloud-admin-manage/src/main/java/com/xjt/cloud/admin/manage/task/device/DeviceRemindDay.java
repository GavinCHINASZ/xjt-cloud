...
@Configuration
@Component
public class DeviceRemindDay implements Job, Serializable {
    private static Logger logger = LoggerFactory.getLogger(TaskOneWeek.class);
    @Override
    public void execute(JobExecutionContext jobExecutionContext) {
        logger.info("ScheduleTask ====> 开启!");
        String exceptionMsg = "设备过期提醒定时任务执行失败";
        try {
            String res = HttpUtils.sendGet(ConstantsClient.DEVICE_REMIND_URL, null);
            if(StringUtils.isNotEmpty(res)){
                Data data = JSONObject.parseObject(res, Data.class);
                if(200 == data.getStatus()){
                    exceptionMsg = "设备过期提醒定时任务执行成功";
                }
            }
        } catch (Exception e) {
