...
/**
 * @ClassName IotCardFlowTask
 * @Description 物联网卡one link流量查询
 * @Author wangzhiwen
 * @Date 2020/9/9 14:38
 **/
@Configuration
@Component
public class IotCardFlowOneLinkTask implements Job, Serializable {
    private static Logger logger = LoggerFactory.getLogger(IotCardFlowOneLinkTask.class);

    @Override
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        logger.info("IotCardFlowOneLinkTask ====> 开启!" );
        ...
        try {
            ...
            if (iotCardService != null){
                iotCardService.iotCardFlowOneLinkTask();
                exceptionMsg = "任务执行成功";
            }
        } catch (Exception e) {
            exceptionMsg = SysLog.getExceptionInfo(e);
            logger.error("IotCardFlowOneLinkTask ====>异常!", e);
        } finally {
            ...
            logger.info("IotCardFlowOneLinkTask ====> 结束!" );
        }
    }
}
