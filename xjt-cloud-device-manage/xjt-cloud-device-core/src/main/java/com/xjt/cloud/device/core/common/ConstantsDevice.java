...
public interface ConstantsDevice {
    ...
    //设备档案导出模版路径,带饼图
    String REPORT_DEVICE_RECORD_PIC_MODEL_FILE_PATH = PropertyUtils.getProperty("report.device.record.pic.model.file.path");
    String REPORT_MODEL_DEVICE_RECORD_PIC = "REPORT_MODEL_DEVICE_RECORD_PIC";//设备档案数据词典
    ...
    //二维码下载模板路径
    String QR_NO_MODEL_FILE_PATH = PropertyUtils.getProperty("qr.no.model.file.path");
    ...
    //巡查点最新巡检记录URL
    String TASK_CHECK_POINT_DEVICE_RECORD_URL = PropertyUtils.getProperty("task.check.point.device.record.url");

    //是否存在平面存点 true存在 false不存在
    String IS_FLOOR_POINT = PropertyUtils.getProperty("isFloorPoint");
    //删除平面存点信息接口路径
    String DEL_FLOOR_POINT_URL = PropertyUtils.getProperty("delFloorPointUrl");
    //删除任务中巡检点信息
    String DEL_TASK_CHECK_POINT = PropertyUtils.getProperty("delTaskCheckPoint");
}
